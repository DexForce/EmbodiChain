# GenSim PR #532-#538 and Task Program Integration Plan

> **Historical design note — superseded.** This proposal predates the final
> Task Program module layout. Its terminology has been normalized to the
> current design: Semantic Calls are internal Task Program instructions and
> lower to executable Atomic Skills. See
> `docs/source/overview/task_program/index.md`.

- Status: superseded
- Last updated: 2026-08-21
- Guiding principle: **Task Program is the sole semantic orchestration layer; Atomic Skills are the executable capability layer**
- Related Task Program foundation PR: [#492](https://github.com/DexForce/EmbodiChain/pull/492)
- Related GenSim PRs:
...
- Related Task Program foundation stack:
...
Task Program and Atomic Skill stack. The adjusted design keeps the parts of
...
#533-#538 should be structurally rewritten around the canonical Task Program
...
| Dynamic scene state | `SceneSnapshot` and `SceneProvider` | Consume snapshots through Task Program semantic layer ports |
...
the Task Program stack. GenSim must not define another semantic-call JSON
format.
...
prefix boundary, that capability belongs in Task Program semantic layer, not in GenSim.
...
- Graph JSON round-trips through the same Task Program semantic-call decoder.
...
- Preflight calls through the canonical Task Program compiler and semantic layer
  catalogs.
...
- Every executable candidate passes Task Program semantic layer provider-free preflight.
- No graph compiler imports or constructs atomic-action implementation types.

### 6.5 PR #536: Thin Task Program semantic layer runtime binding
...
|---|---|
| camera/depth target materialization | registered Task Program semantic layer target provider or lowerer |
...
- Reports retain original Task Program semantic layer events and failures without replacing
...
- Visual target providers to registered Task Program semantic layer grounders/providers.
...
### Gate 0: consolidate the Task Program semantic layer stack
...
```text
Task Program semantic layer runtime consolidation
             |
...
- Semantic graph calls round-trip through the Task Program decoder.
...
- Task Program semantic layer is the only path from semantic intent to `ActionInvocation`.
...
- Effects are committed only from verified Task Program execution results.
...
- The end-to-end tutorials and benchmarks use the same runtime path as Python,
  Task Program, and model-generated callers.

## 13. Expected result

After this adjustment, the two systems become complementary:

- Task Program supplies one reliable, robot-generic, effect-aware, and
