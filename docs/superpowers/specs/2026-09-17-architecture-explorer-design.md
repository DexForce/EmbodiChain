...
状态：范围、数据契约和源码样例已整理；前端、自动提取器及 Sphinx 集成尚未实现。
...
本次交付限于上述范围、数据契约和样例，以及后续实施计划；不启动仿真、
不修改运行时代码、不发布网站。
...
上述清单只确定节点范围；全局视图的关系尚待逐条核对，不从目录邻接推导依赖。
...
建议 React、TypeScript、React Flow，使用 Vite 构建纯静态资源。
依赖版本在前端实施时核对并固定到 lockfile；本次不新增 Node 依赖。

全局视图采用稳定的分组顺序和固定列布局。专题视图先沿用同样布局，节点展开等
