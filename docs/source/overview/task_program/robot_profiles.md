...
recovery, workflow-recovery, runner, action-option, and effect contracts. Task
...
ID must match one provider declared by the immutable Task Program simulation
...
entries whose semantic ID is absent from its call catalog, and Task Program
validates the exact monitor ID/revision before live grounding.

Task Program creates a fresh monitor for every verified grounded call. Pick
...
controller feedback supplies typed pose-relation evidence. The Task Program
...
Task Program lowering uses that binding and the selected preset when constructing
...
monotonic Atomic Skill catalog revision. A later agent-visible action
...
On the standard Task Program path, endpoint adapters and their ordered Gym
transports are declared by `SimulationTaskProgramRegistration`. Adapter
...
See {doc}`../sim/atomic_actions/index` for the direct atomic-action core,
{doc}`index` for declarative semantic contracts,
{doc}`../task_program/index` for semantic task execution, and
