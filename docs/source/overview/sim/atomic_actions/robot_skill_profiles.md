...
time, an explicit `SemanticSkillRuntime.runner_cfg` overrides the selected
preset's runner configuration for every call; otherwise each call keeps its
selected preset's transport timeouts, minimum cycle time, and completion-hold
behavior.
