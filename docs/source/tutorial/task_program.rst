Build an Embodied Task Program
==============================

This tutorial shows how to use Task Program's built-in Semantic Calls. The
...
One runnable semantic task has four pieces:

1. a Task Program YAML file containing object-centric calls;
2. an integration YAML file containing the trusted scene, robot profile,
   policy presets, and allowlisted runtime services;
3. a focused Gym JSON file that describes the environment and references their
   containing directory once with ``task_program_dir``; and
4. the shared ``EmbodiedEnv`` and Task Program adapter that lower the calls
   to Atomic Actions.
...
* ``embodichain_tasks/configs/tasks/manipulation/repeated_pick_place/task_program/integration.yaml``;
* ``embodichain_tasks/configs/tasks/manipulation/repeated_pick_place/env.json``.

The directory contract fixes the two filenames as ``program.yaml`` and
``integration.yaml``. The Gym config does not address either file separately.
...
integration IDs in the program must exactly match the trusted integration.
