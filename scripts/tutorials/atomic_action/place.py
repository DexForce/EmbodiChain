...
class PlaceDemo(DemoBase):
    """Demo that picks up an object and places it at a target end-effector pose."""
    ...
    def setup(self) -> None:
        ...
        self.atomic_engine.register(Place(motion_gen, cfg=place_cfg))

        self.semantics = self._create_object_semantics()
        self.place_eef_poses = self._make_place_eef_poses()

        maybe_open_window(self.sim, self.args)
        if not self.args.no_vis_eef_axis:
            draw_axis_marker(self.sim, "place_target_axis", self.place_eef_poses[-1])

    def run(self) -> None:
        """Plan and replay the PickUp -> Place trajectory."""
        maybe_wait_for_user(
            self.args,
            "Inspect the object, then press Enter to plan PickUp -> Place...",
        )
        ...
        maybe_wait_for_user(self.args, "Press Enter to replay the Place demo...")

        with DemoRecording(self.sim, self.args, prefix="place_auto_play"):
            self._replay_place_trajectory(traj)
    ...
    def _make_place_eef_poses(self) -> torch.Tensor:
        ...
        hover_pose = torch.eye(4, dtype=torch.float32, device=self.sim.device)
        hover_pose[:3, :3] = rotation
        hover_pose[:3, 3] = torch.tensor(
            [-0.40, 0.48, 0.20], dtype=torch.float32, device=self.sim.device
        )
        place_pose = torch.eye(4, dtype=torch.float32, device=self.sim.device)
    ...
    def _replay_place_trajectory(self, traj: torch.Tensor) -> None:
...
def main() -> None:
    """Entry point for the Place demo."""
    setup_print_options()
