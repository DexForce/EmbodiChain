...
"""Demonstrate dual-arm Franka Panda coordinated placement with bread and pan meshes.

The left Franka arm picks up bread. The right Franka arm picks up a pan and moves
it to the lower alignment pose. The left arm places the bread above the pan and
releases it while the right hand keeps holding the pan.

This tutorial uses :class:`~embodichain.lab.sim.robots.DualArmRobotCfg` with
``base_robot="franka"`` and :class:`~embodichain.lab.sim.solvers.PytorchSolver`
...
# Franka Panda 7-DOF home poses (from Isaac Lab reference configuration).
LEFT_ARM_HOME = (0.0, -0.569, 0.0, -2.810, 0.0, 3.037, 0.741)
RIGHT_ARM_HOME = (0.0, -0.569, 0.0, -2.810, 0.0, 3.037, 0.741)
TABLE_TOP_Z = 0.65
...
# Franka Panda hand open/close positions (prismatic finger joints in metres).
FRANKA_FINGER_OPEN = 0.04
FRANKA_FINGER_CLOSE = 0.005
# Mount: side-by-side dual-arm placement with custom arm offsets to match the
# UR5 tutorial's world-space arm positions.
DUAL_MOUNT_CFG = {
    "preset": "side_by_side",
    "separation": 0.6,
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Dual-arm Franka coordinated placement demo"
    )
...
def create_dual_franka_robot(sim: SimulationManager) -> Robot:
    """Create a dual-arm Franka Panda robot via DualArmRobotCfg."""
    cfg = DualArmRobotCfg.from_dict(
        {
            "base_robot": "franka",
            "mount": DUAL_MOUNT_CFG,
            "init_pos": list(ROBOT_INIT_POS),
            "init_rot": list(ROBOT_INIT_ROT),
            "init_qpos": list(LEFT_ARM_HOME)
            + list(RIGHT_ARM_HOME)
            + [FRANKA_FINGER_OPEN, FRANKA_FINGER_OPEN]
            + [FRANKA_FINGER_OPEN, FRANKA_FINGER_OPEN],
        }
    )
...
def get_franka_hand_open_close_qpos(
    robot: Robot,
    hand_control_part: str,
    device: torch.device,
    close_qpos: float = FRANKA_FINGER_CLOSE,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Get open and close qpos for a Franka Panda hand control part.

    The Panda hand has two prismatic finger joints where ``finger_joint2``
    mimics ``finger_joint1``. Only ``finger_joint1`` is actively controlled.
    """
    limits = robot.get_qpos_limits(name=hand_control_part)[0].to(
        device=device, dtype=torch.float32
    )
    hand_open = limits[:, 0].clone()
    hand_open[:] = FRANKA_FINGER_OPEN
...
def run_coordinated_placement_demo(
    ...
    right_open, right_close = get_franka_hand_open_close_qpos(
        robot, "right_hand", sim.device, PAN_HANDLE_CLOSE_QPOS
    )
    left_open, left_close = get_franka_hand_open_close_qpos(
        robot, "left_hand", sim.device
    )
    ...
    recording_started = start_auto_play_recording(
        sim,
        args,
        video_prefix="coordinated_placement_franka_auto_play",
        look_at=(
            (-0.25, 0.0, 2.5),
            (-0.05, 0.0, 0.72),
            (0.0, 0.0, 1.0),
...
def main() -> None:
    """Run the Franka coordinated placement demo."""
    args = parse_arguments()
    sim = initialize_simulation(args)
    robot = create_dual_franka_robot(sim)
