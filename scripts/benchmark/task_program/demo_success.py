...
"""Measure Task Program demo success without retries.

The command line can either aggregate an existing raw artifact or construct one
real Gym environment from explicit Gym and Task Program configurations. Live
...
Run offline:
``python -m scripts.benchmark.task_program.demo_success --raw-json RAW``

Run live:
``python -m scripts.benchmark.task_program.demo_success --run-simulation
--gym_config GYM --task-program PROGRAM --case-id CASE --seeds 0 1
--raw-json RAW``
"""
...
_BENCHMARK_ID = "task_program_demo_success"
...
def load_raw_trials(path: str | Path) -> tuple[DemoSuccessTrial, ...]:
    ...
    if data.get("benchmark") != _BENCHMARK_ID:
        raise ValueError("Raw JSON is not an Task Program demo success artifact.")
...
def write_markdown_report(path: str | Path, aggregates: DemoSuccessAggregates) -> Path:
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Task Program Demo Success Benchmark",
        "",
...
def _create_gym_demo_success_environment(
    ...
    if getattr(launcher_args, "action_config", None) is not None:
        raise ValueError(
            "--action_config is not supported by the Task Program benchmark."
        )
...
def run_gym_demo_success_benchmark(
    ...
    *,
    ...
    Args:
        ...
        task_program_path: Explicit Task Program JSON/YAML configuration.
    ...
    try:
        env = factory(launcher_args, task_program_path)
...
def run_all_benchmarks(
    ...
    *,
    ...
    """
    print("=" * 60)
    print("Task Program Demo Success Benchmark")
    print("=" * 60)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run one fixed-seed Task Program benchmark or aggregate an "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Task Program JSON/YAML file used by --run-simulation.",
    )
    parser.add_argument(
        "--case-id",
        type=str,
...
def _validate_cli_mode(
    ...
    *,
    ...
    live_values = {
        ...
        "--task-program": args.task_program,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.run_simulation:
        ...
        run_gym_demo_success_benchmark(
            case,
            launcher_args=args,
            task_program_path=args.task_program,
