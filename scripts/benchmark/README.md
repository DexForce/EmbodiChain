...
当前交付是一个可用的核心子集，以及使用它的 `camera-pilot` 相机实验。
它覆盖本地隔离运行、原始计时、产物协议、来源追踪、离线统计和比较条件检查。
...
scripts/benchmark/
  ...
  core/
    ...
    measurement.py             通用同步计时；不依赖图像、Torch 或仿真器
    artifacts.py               严格/原子 JSON 写入、配置哈希、运行目录
    provenance.py              软件版本、源文件哈希与设备快照
  reporting/
    aggregation.py             按平台和工作负载聚合；原始单位/分母保留
    comparison.py              检查质量、硬件、配置、计时边界是否可比
  rendering/
...
每次执行包含 `manifest.json`、`config.json`、`runs.json`；每个 worker 包含
`result.json` 和 `worker.log`，以及领域附加产物。`manifest.json` 记录实验、
case/run/repeat 身份、命令和超时；完整环境变量不会写入计划。
...
项目的 `@configclass`。核心当前不负责集群、候选生成、Gym reset 或数据集 commit。
...
增加一个领域配置、一个 worker、平台适配器及领域报告模板即可。launcher 使用
`RunSpec` / `run_experiment`，worker 使用 `measure_loop` 和公共 artifact/provenance
函数，报告使用 `MetricDefinition` / `aggregate_runs` / `comparison_reasons`。
