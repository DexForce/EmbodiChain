...
class ViserBackend(VisualizationBackend):
    ...
    def publish_frame(self, frame: SceneFrame) -> bool:
        ...
        frame_index = {node_id: index for index, node_id in enumerate(frame.node_ids)}
        ...
        camera_index = {
            camera_id: index for index, camera_id in enumerate(frame.camera_ids)
        }
        for camera_id, handle in self._camera_handles.items():
