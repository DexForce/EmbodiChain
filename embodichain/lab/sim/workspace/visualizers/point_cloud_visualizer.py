...
class PointCloudVisualizer(BaseVisualizer):
    ...
    def visualize(
        ...
        **kwargs: Any,
        ...
        Returns:
            Backend-specific point-cloud handle, Viser overlay, geometry, or
            matplotlib figure.
        ...
        if self.backend == "data":
            ...
            overlay = self._create_viser_point_cloud(points, colors, point_size)
            self._last_visualization = {
                "point_cloud_overlay": overlay,
                "point_size": point_size,
            }
            return overlay
    ...
    def _create_viser_point_cloud(
        ...
        """Publish a persistent point-cloud overlay through SimulationManager."""
        if self.sim_manager is None:
            raise ValueError("sim_manager is required for 'viser' backend")
        from embodichain.lab.visualization import PointCloudOverlay, SceneOverlays

        colors_uint8 = np.rint(np.clip(colors, 0.0, 1.0) * 255.0).astype(np.uint8)
        overlay = PointCloudOverlay(
            overlay_id=f"workspace_{self.control_part_name or 'default'}",
            points=np.asarray(points, dtype=np.float32),
            colors=colors_uint8,
            point_size=point_size,
        )
        self.sim_manager.set_visualization_overlays(
            SceneOverlays(point_clouds=(overlay,))
        )
        logger.log_info(
            f"Published Viser point cloud '{overlay.overlay_id}' with "
            f"{len(points)} points (point_size={point_size})"
        )
        return overlay
