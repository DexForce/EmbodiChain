...
Each action inherits :class:`AtomicAction` directly and composes a
:class:`TrajectoryBuilder` for shared trajectory math. ``execute`` takes a typed
target plus a :class:`WorldState` and returns an :class:`ActionResult` whose
trajectory is full-robot DoF shaped ``(n_envs, n_waypoints, robot.dof)``.
"""
...
class CoordinatedPlacementCfg(ActionCfg):
    ...
    """Combined control part containing placing and support arm joints."""

    placing_arm_control_part: str = "left_arm"
    """Arm that places and releases the upper object."""

    support_arm_control_part: str = "right_arm"
    """Arm that holds the lower support object."""
    ...
    """Placing hand qpos for the open state."""

    placing_hand_close_qpos: torch.Tensor | None = None
    """Placing hand qpos for the closed state."""

    support_hand_open_qpos: torch.Tensor | None = None
    """Support hand qpos for the open state."""

    support_hand_close_qpos: torch.Tensor | None = None
    """Support hand qpos for the closed state."""
...
class CoordinatedPlacement(AtomicAction):
    """Move two held objects together, release the placing hand, keep support held."""
    ...
    def __init__(
        ...
        self._init_dual_arm_parts(
            first_arm_control_part=self.cfg.placing_arm_control_part,
            second_arm_control_part=self.cfg.support_arm_control_part,
            first_hand_control_part=self.cfg.placing_hand_control_part,
            second_hand_control_part=self.cfg.support_hand_control_part,
        )
        self.placing_arm_joint_ids = self.first_arm_joint_ids
        self.support_arm_joint_ids = self.second_arm_joint_ids
        self.placing_hand_joint_ids = self.first_hand_joint_ids
        self.support_hand_joint_ids = self.second_hand_joint_ids

        self._validate_hand_qpos_cfg(
            (
                "placing_hand_open_qpos",
                "placing_hand_close_qpos",
                "support_hand_open_qpos",
                "support_hand_close_qpos",
            ),
            "CoordinatedPlacementCfg",
        )
        self.placing_hand_open_qpos = self._expand_qpos(
            self.cfg.placing_hand_open_qpos,
            self.first_hand_dof,
            "placing_hand_open_qpos",
        )
        self.placing_hand_close_qpos = self._expand_qpos(
            self.cfg.placing_hand_close_qpos,
            self.first_hand_dof,
            "placing_hand_close_qpos",
        )
        self.support_hand_open_qpos = self._expand_qpos(
            self.cfg.support_hand_open_qpos,
            self.second_hand_dof,
            "support_hand_open_qpos",
        )
        self.support_hand_close_qpos = self._expand_qpos(
            self.cfg.support_hand_close_qpos,
            self.second_hand_dof,
            "support_hand_close_qpos",
        )

    def _validate_hand_qpos_cfg(self, names: tuple[str, ...], cfg_name: str) -> None:
        """Ensure all hand state tensors are provided."""
        for name in names:
            if getattr(self.cfg, name) is None:
                logger.log_error(f"{name} must be specified in {cfg_name}", ValueError)
    ...
    def _resolve_object_pose(
        ...
        pose = self._resolve_pose(pose, name)
        return self.builder.apply_local_offset(
            pose,
            torch.tensor([0.0, 0.0, height_offset], device=self.device),
        )

    def _resolve_object_to_eef(
        self, held_state: HeldObjectState, name: str
    ) -> torch.Tensor:
    ...
    def _compute_segment_lengths(self, release: bool) -> dict[str, int]:
        ...
        if n_approach < 2:
            logger.log_error(
                "Not enough waypoints for coordinated placement. Please increase "
    ...
    def execute(
        ...
        placing_start_qpos, support_start_qpos = self._resolve_dual_arm_start(state)
        ...
        placing_approach_targets = torch.stack([placing_lift_xpos, placing_xpos], dim=1)
        support_approach_targets = support_xpos.unsqueeze(1)
        ok, placing_approach_traj = self._plan_named_arm_trajectory(
            self.cfg.placing_arm_control_part,
            placing_start_qpos,
            placing_approach_targets,
            segments["approach"],
        )
        if not ok:
            return self._fail(state)
