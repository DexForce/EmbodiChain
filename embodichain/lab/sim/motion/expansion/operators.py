...
def _reject_limit_violations(
    positions: torch.Tensor, limits: torch.Tensor, indices: list[int]
) -> None:
    """Reject a proposal only for joints the operator actually moved.

    An observed reference can hold a joint a few microradians outside its
    declared range. Checking untouched joints would blame the sampled residual
    for that pre-existing artifact and reject every proposal.
    """
...
def via_points(
    ...
    *,
    ...
    this operator places ``via_count`` interior knots and interpolates them
    with clamped cubic Hermite segments. Two or more knots admit sign changes
    along a phase, so the resulting paths differ in shape rather than only in
    amplitude. Phase endpoints and their derivatives are unchanged, which keeps
    every annotated waypoint exact.

    All knots displace along one sampled joint-space direction, and only their
    signed magnitudes vary. Sampling each joint of each knot independently
    instead would decorrelate the joints, and the tool path that comes out of
    forward kinematics then wanders: measured on a pick-and-place reference it
    grew about half as long again, without adding anything a caller wants.
    ...
    """
    phases = allowed_phases(template, "via_points")
    ...
    for phase in phases:
        ...
        magnitudes = 2 * values[:via_count] - 1
        direction = 2 * values[via_count:] - 1
        knots = magnitudes[:, None] * (direction * scale * spans)[None, :]
...
def nullspace_residual(
    ...
    *,
    ...
    """
    phases = allowed_phases(template, "nullspace_residual")
    scale = _normalized_scale(normalized_scale)
