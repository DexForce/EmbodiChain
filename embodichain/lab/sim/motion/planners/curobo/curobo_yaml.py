    ...
    *,
    ...
    Args:
        rigid_objects: Live obstacles whose physical shapes define the world.
            Mapping keys are authoritative obstacle IDs. A sequence derives
            each ID from the object's ``uid`` (or ``obstacle_<index>``).
        env_id: Environment row used for geometry and initial poses.
        representation: Global ``auto`` or forced representation policy.
        overrides: Per-object policies keyed by canonical obstacle ID.
        dynamic_obstacle_names: Canonical obstacle IDs whose poses change
            between plans.
    ...
    for object_name, obj in named_rigid_objects:
        if (
        ...
        if object_name in object_names:
            raise ValueError(
                f"Duplicate obstacle name {object_name!r}; obstacle IDs must be unique."
            )
        ...
        if not shapes:
            if registry_backed:
                raise ValueError(
                    f"Registry-backed obstacle {object_name!r} has no physical "
                    "collision shapes; the declared collision world cannot omit it."
                )
            logger.log_warning(
                f"RigidObject {object_name!r} has no physical collision shapes; "
                "skipping collision export."
            )
            continue
        object_pose = (
            torch.as_tensor(
    ...
    if unknown_overrides:
        raise ValueError(
            f"representation overrides reference unknown obstacle IDs: "
            f"{unknown_overrides}."
        )
    ...
    *,
    ...
    Args:
        rigid_objects: Live simulator obstacles represented by the cache,
            optionally keyed by canonical registry ID.
...
def visualize_curobo_collision_models(
    robot: Robot,
    robot_yaml_path: str,
    rigid_objects: Sequence[RigidObject] | Mapping[str, RigidObject] | None = None,
    ...
    Args:
        robot: Live simulator robot whose link poses position the cached spheres.
        robot_yaml_path: Generated cuRobo robot YAML containing collision spheres.
        rigid_objects: Optional live world obstacles represented by ``world_scene``.
        world_scene: Optional tensor-backed scene or cuRobo ``Scene`` instance.
        env_id: Simulator environment instance to visualize.
    """
    import dexsim
    import yaml
