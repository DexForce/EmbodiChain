...
def _default_mujoco_mimic_solref(physics_dt: float, num_substeps: int) -> np.ndarray:
    ...
    Positive MuJoCo ``solref`` uses ``(timeconst, dampratio)`` and therefore
    retains the effective-mass scaling of PhysX articulation mimic joints.
    MuJoCo's reference-safety rule clamps ``timeconst`` to twice the solver
    timestep, so apply the same bound explicitly.
    """
...
def _configure_newton_mimic_compliance(
    *,
    ...
    if runtime is None:
        raise RuntimeError("Newton Spawn articulation has no finalized runtime.")
