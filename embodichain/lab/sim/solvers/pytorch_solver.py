...
class PytorchSolver(BaseSolver):
    ...
    def get_ik(
        ...
        **kwargs,
        ...
        tcp_xpos_inv = torch.eye(4, device=self.device, dtype=torch.float32)
        tcp_rotation_inv = tcp_xpos[:3, :3].T
        tcp_xpos_inv[:3, :3] = tcp_rotation_inv
        tcp_xpos_inv[:3, 3] = -tcp_rotation_inv @ tcp_xpos[:3, 3]
