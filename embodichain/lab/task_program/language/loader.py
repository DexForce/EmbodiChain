...
"""Safe file and strict JSON loading for declarative Task Programs."""
...
MAX_TASK_PROGRAM_BYTES = 4 * 1024 * 1024
"""Maximum serialized Task Program size accepted by the file loader."""
...
def _validate_decoded_json_unicode(value: object) -> None:
    ...
    if type(value) is str:
        try:
            ...
            raise _StrictJsonValueError(
                "invalid_utf8",
                "Task Program JSON contains an unpaired Unicode surrogate.",
            ) from error
        return
    ...
    *,
    ...
    try:
        ...
        raise TaskProgramDecodeError(
            "invalid_utf8",
            (),
            "Task Program JSON must be valid UTF-8 text.",
        ) from error
    if len(payload) > max_bytes:
        raise TaskProgramDecodeError(
            "input_too_large",
            (),
            f"Task Program JSON exceeds the {max_bytes}-byte input limit.",
        )
    try:
        value = json.loads(
            text,
        ...
        raise TaskProgramDecodeError(error.code, (), error.message) from error
    except json.JSONDecodeError as error:
        raise TaskProgramDecodeError(
            "invalid_json",
            (),
            "Invalid Task Program JSON at "
        ...
        raise TaskProgramDecodeError(
            "input_too_deep",
            (),
            "Task Program JSON exceeds the parser nesting limit.",
        ) from error
    except ValueError as error:
        raise TaskProgramDecodeError(
            "invalid_json",
            (),
            "Task Program JSON contains an invalid numeric value.",
        ) from error


def parse_task_program_json(
    text: str,
    *,
    max_bytes: int = MAX_TASK_PROGRAM_BYTES,
) -> dict[str, object]:
    ...
    fields before calling :func:`decode_task_program`. It rejects duplicate
    ...
    not validate the Task Program schema.
...
def loads_task_program_json(
    ...
    *,
    ...
    are rejected before the existing Task Program decoder is called.
...
def load_task_program(
    ...
    *,
    ...
    Raises:
        ...
        TaskProgramValidationError: If ``validation_context`` rejects an
            external reference.
    ...
    if not program_path.is_file():
        raise FileNotFoundError(f"Task Program path is not a file: {program_path}.")
    suffix = program_path.suffix.lower()
    if suffix not in {".json", ".yaml", ".yml"}:
        raise ValueError(
            "Task Program must use a .json, .yaml, or .yml extension; "
            f"got {program_path.name!r}."
        )

    payload = program_path.read_bytes()
    if len(payload) > MAX_TASK_PROGRAM_BYTES:
        raise TaskProgramDecodeError(
            "input_too_large",
            (),
            "Task Program exceeds the " f"{MAX_TASK_PROGRAM_BYTES}-byte input limit.",
        )
