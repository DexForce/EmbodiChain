...
"""Strict JSON/YAML-value decoder for declarative Task Programs."""
...
class TaskProgramConfigError(ValueError):
...
class TaskProgramDecodeError(TaskProgramConfigError):
    """Raised when untrusted data does not match a supported strict schema."""


class TaskProgramValidationError(TaskProgramConfigError):
    """Raised when an explicit static integration context rejects a reference."""


@runtime_checkable
class TaskProgramValidationContext(Protocol):
...
def _error(code: str, path: ConfigPath, message: str) -> TaskProgramDecodeError:
    """Build one decoder diagnostic."""
    return TaskProgramDecodeError(code, path, message)
...
def validate_task_program(
    ...
    context: TaskProgramValidationContext,
) -> None:
    """Resolve external references without observing or executing an environment.

    Args:
        config: Fully decoded and internally validated Task Program.
    ...
    if not isinstance(context, TaskProgramValidationContext):
        raise TypeError("context must implement TaskProgramValidationContext exactly.")
    _call_context(
    ...
    for node, path in _walk_program(config.program, path=("program",)):
        if type(node) is InvokeCfg:
            ...
            call_path = (*path, "call")
            _call_context(
                context.validate_semantic_call,
                call,
                path=call_path,
            )
...
def decode_task_program(
    ...
    *,
    ...
    assert type(config) is TaskProgramCfg
