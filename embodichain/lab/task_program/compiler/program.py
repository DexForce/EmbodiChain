...
"""Provider-free compilation and lazy expansion of Task Program ASTs."""
...
from ..language.decoder import ConfigPath, TaskProgramConfigError, render_config_path
...
class TaskProgramCompileError(TaskProgramConfigError):
...
class CompiledParallelBranch:
    ...
    branch_index: int
    calls: tuple[CompiledTaskProgramCall, ...]
    source_path: ConfigPath
...
@dataclass(frozen=True, slots=True)
class CompiledTaskProgramSegment:
    ...
    def __post_init__(self) -> None:
        ...
        if not calls or not all(
            ...
            raise TypeError(
                "calls must contain at least one exact CompiledTaskProgramCall."
            )
...
@dataclass(frozen=True, slots=True)
class CompiledTaskProgramAnalysis:
...
def _instantiate_call(
    ...
    *,
    ...
    return CompiledTaskProgramCall(
...
def _iter_segments(
    ...
    *,
    ...
    if type(template) is _InvokeTemplate:
        ...
        state.call_index += 1
        segment = CompiledTaskProgramSegment(
    ...
    validators: list[CompiledTaskProgramValidator] = []
...
class CompiledTaskProgram:
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        """Reject construction outside :class:`TaskProgramCompiler`."""
        del args, kwargs
        raise TypeError(
            "CompiledTaskProgram values are created by TaskProgramCompiler."
        )
    ...
    def _create(
        ...
        *,
        ...
        if not values or not all(
            type(segment) is CompiledTaskProgramSegment for segment in values
        ):
            raise TypeError(
                "segments must contain at least one CompiledTaskProgramSegment."
            )
    ...
    @property
    def integration(self) -> TaskProgramIntegrationCfg:
    ...
    def iter_segments(self) -> Iterator[CompiledTaskProgramSegment]:
        """Iterate the already materialized provider-free segments."""
        return iter(self._segments)

    def preflight_analyses(self) -> tuple[CompiledTaskProgramAnalysis, ...]:
        ...
        """
        analyses: list[CompiledTaskProgramAnalysis] = []
        stretch: list[CompiledTaskProgramSegment] = []
    ...
    def sequential_execution_analysis(
        ...
        window: list[CompiledTaskProgramSegment] = []
        ...
        return CompiledTaskProgramAnalysis(
            analysis_id=(
    ...
    def __iter__(self) -> Iterator[CompiledTaskProgramSegment]:
...
def _materialize_program(
    *,
    ...
    segments: list[CompiledTaskProgramSegment] = []
    expanded_calls = 0
...
class TaskProgramCompiler:
    ...
    @classmethod
    def from_scene_registry(cls, registry: SceneRegistry) -> TaskProgramCompiler:
    ...
    def compile(self, config: TaskProgramCfg) -> CompiledTaskProgram:
        """Compile one validated AST into a bounded provider-free program.

        Args:
            config: Strict, supported-version Task Program configuration.

        Returns:
            Immutable segments with repeat-local targets already resolved.

        Raises:
            TaskProgramCompileError: If typed scene resolution or AST lowering
                fails.
        """
