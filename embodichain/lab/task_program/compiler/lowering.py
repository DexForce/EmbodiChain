...
from embodichain.lab.task_program.semantics._validation import (
...
class AnalyzedSemanticCall:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.effect_assurance, EffectAssurance):
...
class SemanticWorkflow:
    ...
    def __init__(self, *args: object, **kwargs: object) -> None:
        """Reject construction outside :class:`SemanticCallCompiler`."""
        del args, kwargs
        raise TypeError(
            "SemanticWorkflow values are created by " "SemanticCallCompiler.analyze()."
        )
...
class SemanticCallCompiler:
        ...
        for index, bound in enumerate(bound_calls):
            ...
            analyzed.append(
                AnalyzedSemanticCall(
                    ...
                    effect_assurance=bound.preset.effect_assurance,
        ...
        *,
        ...
        if workflow.skill_catalog_revision != engine.skill_catalog_revision:
            raise _diagnostic(
                "semantic_catalog_stale",
                path,
                "The installed Atomic Skill catalog changed after workflow "
                "analysis.",
            )
    ...
    def _effect_monitor_ref(
        ...
        *,
        ...
        if bound.preset.effect_assurance is EffectAssurance.PROJECTED:
            if monitor_ref is not None:  # pragma: no cover - preset validates this
                raise AssertionError("Projected presets cannot select monitors.")
            return None
        if monitor_ref is None:
            if type(bound.linked.call) in {Pick, Place, HandOver}:
                raise _diagnostic(
