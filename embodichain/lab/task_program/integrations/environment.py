...
"""Explicit production assembly for environment-backed Task Programs.
...
from embodichain.lab.task_program.runtime.assembly import (
...
@runtime_checkable
class TaskProgramAdapterFactory(Protocol):
...
@runtime_checkable
class TaskProgramEnvironmentFactory(Protocol):
...
@runtime_checkable
class _RegistrationOwningTaskProgramFactory(Protocol):
    """Internal capability exposing one exact standard registration owner."""

    @property
    def task_program_registration(self) -> SimulationTaskProgramRegistration:
...
class TaskProgramRuntimeAssembly:
    ...
    Attributes:
        ...
        compiler: Bound Semantic Call compiler.
        ...
        runtime: Nonblocking Semantic Call runtime.
    """
...
class TaskProgramEnvironmentAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(factory, TaskProgramEnvironmentFactory):
            raise TypeError("factory must implement TaskProgramEnvironmentFactory.")
        ...
        if (
            ...
            and type(integration_catalog) is not TaskProgramIntegrationCatalog
        ):
            raise TypeError(
                "integration_catalog must be exactly "
                "TaskProgramIntegrationCatalog or None."
            )
        if (
            registration is not None
            and type(registration) is not SimulationTaskProgramRegistration
        ):
            raise TypeError(
        ...
        if registration is not None:
            if not isinstance(factory, _RegistrationOwningTaskProgramFactory):
                raise TypeError(
            ...
            owned_registration = registration_owner.task_program_registration
            if type(owned_registration) is not SimulationTaskProgramRegistration:
                raise TypeError(
    ...
    def compile(self, program: TaskProgramCfg) -> CompiledTaskProgram:
        ...
        """
        if type(program) is not TaskProgramCfg:
            raise TypeError("program must be exactly TaskProgramCfg.")
        ...
        return TaskProgramCompiler.from_scene_registry(registry).compile(program)
    ...
    def _assemble_semantic_components(
        ...
        if self._registration is not None:
            ...
            if not isinstance(
                registration_owner,
                _RegistrationOwningTaskProgramFactory,
            ):
    ...
    def _assemble_execution_runtime(
        ...
        return TaskProgramRuntimeAssembly(
    ...
    def _preflight_program(
        ...
        if any(analysis.kind == "parallel_branch" for analysis in analyses) and (
            ...
            raise ValueError(
                "Task Programs containing parallel blocks require an explicit "
    ...
    def _validate_selection(
        ...
        if integration.scene_registry != self._scene_registry_id:
            raise ValueError(
                f"Task Program selects scene_registry "
        ...
        if integration.robot_profile != self._robot_profile_id:
            raise ValueError(
                f"Task Program selects robot_profile "
    ...
    def _validate_registration_ownership(self) -> None:
        ...
        if registration is None:
            return
        if not isinstance(self._factory, _RegistrationOwningTaskProgramFactory):
            raise IntegrationFingerprintMismatch(
                "The standard environment factory no longer exposes registration "
                "ownership."
            )
        current = self._factory.task_program_registration
        if type(current) is not SimulationTaskProgramRegistration:
            raise IntegrationFingerprintMismatch(
