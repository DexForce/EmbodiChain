...
class EffectAssurance(str, Enum):
...
_EMPTY_EFFECT_MONITORS: Mapping[str, EffectMonitorRef] = MappingProxyType({})
...
class WorkflowRecoveryPolicy:
    ...
    returns control to the Task Program workflow executor.
...
class SkillPolicyPreset:
    ...
    effect_assurance: EffectAssurance
    ...
    def __init__(
        ...
        *,
        ...
        effect_assurance: EffectAssurance,
        effect_monitors: Mapping[str, EffectMonitorRef] = _EMPTY_EFFECT_MONITORS,
        ...
        if not isinstance(effect_assurance, EffectAssurance):
            raise TypeError("effect_assurance must be an EffectAssurance.")
        ...
        if not isinstance(effect_monitors, Mapping):
            raise TypeError("effect_monitors must be a mapping.")
        if effect_assurance is EffectAssurance.PROJECTED and effect_monitors:
            raise ValueError(
                "Projected presets cannot declare effect monitors; select "
                "verified assurance when physical evidence is required."
            )
        normalized_effect_monitors: dict[str, EffectMonitorRef] = {}
        for semantic_id, monitor_ref in effect_monitors.items():
            _validate_identifier(
                semantic_id,
        ...
        object.__setattr__(self, "effect_assurance", effect_assurance)
    ...
    def effect_monitors(self) -> Mapping[str, EffectMonitorRef]:
        ...
        Verified presets require a monitor for every curated effectful call they
        compile. Projected presets require this mapping to be empty.
        """
    ...
    def snapshot(self) -> SkillPolicyPreset:
        ...
        return SkillPolicyPreset(
            ...
            effect_assurance=self.effect_assurance,
...
class BoundRobotSkillProfile:
    ...
    def _assert_catalog_current(self) -> None:
        ...
        if self._engine.skill_catalog_revision != self._skill_catalog_revision:
            raise RuntimeError(
                "AtomicActionEngine skill catalog changed after the robot skill "
    ...
    def _validate_named_skill_configuration(self) -> None:
        """Reject defaults and preset selections for absent Atomic Skills."""
