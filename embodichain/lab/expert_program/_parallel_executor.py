...
from embodichain.lab.semantic_skills.calls import SemanticCallSpec
from embodichain.lab.semantic_skills.effects import SymbolicStateKey
...
from ._semantic_compiler import SemanticCallCompiler
from ._semantic_executor import SemanticCallExecutor
...
@runtime_checkable
class ParallelBranchExecutorPort(Protocol):
...
class ParallelExecutorBranch:
    """One semantic-call lane and its exclusive resource claim."""

    branch_id: str
    calls: tuple[SemanticCallSpec, ...]
    claim: ResourceClaim
    executor: ParallelBranchExecutorPort = field(repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.executor, ParallelBranchExecutorPort):
            raise TypeError("executor must implement ParallelBranchExecutorPort.")
...
class ParallelSemanticExecutionResult:
    ...
    branch_results: Mapping[str, SemanticExecutionResult]
    elapsed_steps: int
    command_count: int
    ...
    def __post_init__(self) -> None:
        ...
        branches: dict[str, SemanticExecutionResult] = {}
        for branch_id, result in self.branch_results.items():
            _validate_identifier(branch_id, field_name="branch result IDs")
            if not isinstance(result, SemanticExecutionResult):
                raise TypeError(
                    "branch_results values must be SemanticExecutionResult values."
                )
...
class ParallelSemanticExecutor:
    ...
    def __init__(
        self,
        branches: tuple[ParallelExecutorBranch, ...],
        ...
        *,
        ...
        if not isinstance(branches, tuple) or len(branches) < 2:
            raise ValueError("ParallelSemanticExecutor requires at least two branches.")
        if not all(type(branch) is ParallelExecutorBranch for branch in branches):
            raise TypeError("branches must contain ParallelExecutorBranch values.")
        ...
        initial = branches[0].executor.result
        for branch in branches[1:]:
            result = branch.executor.result
            if (
    ...
    def from_template(
        cls,
        template_runtime: SemanticCallExecutor,
        branch_calls: Mapping[str, tuple[SemanticCallSpec, ...]],
        command_sink: CommandSink,
        timing_policy: ParallelTimingPolicy,
        safety_validator: ParallelCommandSafetyValidator,
        *,
        ...
        Returns:
            ...
            state.
        """
        if not isinstance(template_runtime, SemanticCallExecutor):
            raise TypeError("template_runtime must be a SemanticCallExecutor.")
        if template_runtime.status is SemanticExecutionStatus.RUNNING:
            raise RuntimeError("template_runtime must not be running.")
        branches: list[ParallelExecutorBranch] = []
    ...
    def result(self) -> ParallelSemanticExecutionResult:
        ...
        return ParallelSemanticExecutionResult(
            ...
            branch_results={
                branch.branch_id: branch.executor.result for branch in self._branches
            },
    ...
    def start(
        ...
        *,
        ...
        if self._status is not SemanticExecutionStatus.IDLE:
            raise RuntimeError("ParallelSemanticExecutor instances are one-shot.")
        _validate_identifier(workflow_id, field_name="workflow_id")
        ...
        started: list[ParallelExecutorBranch] = []
        try:
            for branch in self._branches:
                branch.executor.start(
                    *branch.calls,
        ...
        try:
            ...
            if self._deferred_frame is not None:
                ...
                if (
                    ...
                    and self._status is SemanticExecutionStatus.RUNNING
                    and (
            ...
            if self._terminal_hold_pending:
                ...
                return self.result
            for branch in self._branches:
                if not branch.executor.result.terminal:
    ...
    def _timeout_pending_rows(self) -> None:
        ...
        errors: list[str] = []
        for branch in self._branches:
            if branch.executor.result.terminal:
                continue
            try:
                branch.executor.cancel(self._message)
            except Exception as exc:
    ...
    def _sync_branch_identity(self) -> None:
        ...
        reference = self._branches[0].executor.result
    ...
    def _update_barrier(self) -> None:
        ...
        if new_failure.any():
            ...
            for branch in self._branches:
                ...
                if mask.any():
                    branch.executor.deactivate_rows(mask, reason=reason)
    ...
    def _merge_verified_state(self) -> bool:
        ...
        effects = {
            branch.branch_id: (
                _task_state_delta(
                    self._initial_state,
                    branch.executor.result.task_state,
                ),
    ...
    def _finish_if_complete(self) -> None:
        ...
        self._merge_verified_state()
        if (
            self._status is SemanticExecutionStatus.RUNNING
            and not self._terminal_stop_forwarded
        ):
