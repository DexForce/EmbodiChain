...
# TODO: to be depercated.
def set_camera_pose(sensor: dexsim.sensor.Sensor, pose: np.ndarray) -> np.ndarray:
    actual_camera_pose = deepcopy(pose)
    if type(sensor).__name__ == "BinocularCam":
        actual_camera_pose[:3, 1] = -actual_camera_pose[:3, 1]
        actual_camera_pose[:3, 2] = -actual_camera_pose[:3, 2]
    sensor.set_world_pose(actual_camera_pose)
    return pose


# TODO: to be depercated.
def set_camera_pose_look_at(
    sensor: dexsim.sensor.Sensor,
    from_point: np.ndarray,
    to_point: np.ndarray,
    up_vector: np.ndarray = np.array([0, 0, 1]),
) -> np.ndarray:
    """set camera pose using look at

    Args:
        sensor (dexsim.sensor.Sensor): dexsim sensor
        from_point (np.ndarray): camera position
        to_point (np.ndarray): camera look at position
        up_vector (np.ndarray, optional): camera up vector. Defaults to np.array([0, 0, 1]).

    Returns:
        np.ndarray: camera pose
    """
    from dexsim.utility.cam_utils import look_at_to_pose

    camera_pose = look_at_to_pose(from_point, to_point, up_vector)
    actual_camera_pose = deepcopy(camera_pose)
    if type(sensor).__name__ == "BinocularCam":
        camera_pose[:3, 1] = -camera_pose[:3, 1]
        camera_pose[:3, 2] = -camera_pose[:3, 2]
    sensor.set_world_pose(camera_pose)
    return actual_camera_pose
