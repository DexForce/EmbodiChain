...
"""Simulation-backed Expert Program post-policies and validators.
...
from .simulation import SimulationSceneBinding
...
class SimulationSegmentPolicyPort:
    ...
    def _validate_segment_membership(
        segment: Any,
        member: CompiledPostPolicy | CompiledProgramValidator,
        *,
        kind: str,
    ) -> None:
        """Require the supplied compiled value to belong to the exact segment."""
        if type(segment) is not CompiledProgramSegment:
            raise TypeError("segment must be exactly CompiledProgramSegment.")
        values = (
