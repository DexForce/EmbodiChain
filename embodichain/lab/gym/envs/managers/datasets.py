...
class LeRobotRecorder(Functor):
    ...
    def _build_features(self) -> Dict:
        ...
        # Add episode-constant observation features from the episode obs space
        if self._env._episode_obs_space is not None:
            for key, space in self._env._episode_obs_space.spaces.items():
                if isinstance(space, gym.spaces.Dict):
                    self._add_nested_features(features, key, space)
                    continue
                # Handle empty shapes for scalar values
    ...
    def _modify_feature_names(self, features: dict[str, Any]) -> None:
        """Modify feature names based on functor configuration.

        Handles two post-processing steps:
        1. Convert empty ``()`` shapes to ``(1,)`` for LeRobot compatibility.
        2. Assign meaningful names to ``get_object_uid`` observations based on
           asset properties (object UID for RigidObject, link names for
           Articulation/Robot).
        """
        ...
        # Assign meaningful names based on asset properties for get_object_uid
    ...
    def _convert_frame_to_lerobot(
        ...
        Args:
            ...
            task: Task name
            env_id: Environment index for reading episode-constant observations.
        ...
        # Add episode-constant observations from the flat episode_obs buffer.
        # These values are the same for every frame in the episode but are
        # included in each frame for compatibility with standard training
        # pipelines that expect per-frame data.
        if self._env.episode_obs_buffer is not None:
            self._add_episode_obs_to_frame(frame, self._env.episode_obs_buffer, env_id)
    ...
    def _add_episode_obs_to_frame(
        ...
        Reads values from the flat ``episode_obs`` buffer (no time dimension)
        and adds them to the frame dict. These values are the same for every
        frame in the episode.

        Args:
            frame: The frame dict to update.
            episode_obs: The flat episode observation buffer (shape
                ``[num_envs, *obs_shape]``).
            env_id: The environment index to read from.
        """
