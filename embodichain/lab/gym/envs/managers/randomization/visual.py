...
from typing import TYPE_CHECKING, Literal, Mapping, Union, Dict
...
def _select_texture_indices(
    ...
    target_ids: list[int],
    num_textures: int,
    texture_indices: Mapping[int | str, int] | None,
) -> list[int]:
    """Select one library texture index for each target environment."""
    if num_textures <= 0:
        raise ValueError("At least one texture source is required")

    if mode == "random":
        return torch.randint(num_textures, (len(target_ids),)).tolist()
    if mode == "without_replacement":
        if len(target_ids) > num_textures:
            raise ValueError(
                "without_replacement requires at least one texture per target"
            )
        return torch.randperm(num_textures)[: len(target_ids)].tolist()
    if mode == "cycle":
        return [index % num_textures for index in range(len(target_ids))]
    if mode == "fixed":
        ...
        for env_id in target_ids:
            if env_id in texture_indices:
                index = texture_indices[env_id]
            elif str(env_id) in texture_indices:
                index = texture_indices[str(env_id)]
            else:
                raise ValueError(
...
class randomize_visual_material(Functor):
    ...
    def __call__(
        ...
        texture_sampling: Literal[
            "random", "without_replacement", "cycle", "fixed"
        ] = "random",
        texture_indices: Mapping[int | str, int] | None = None,
    ):
        if self._new_mode:
    ...
    def _call_reuse(
        ...
        texture_plan = (
            _select_texture_indices(
                texture_sampling,
                selected_env_ids,
                len(self._library_textures),
                texture_indices,
            )
            if self._library_textures and texture_sampling != "random"
            else [None] * len(selected_env_ids)
        )

        def _apply(
            reuse_i: int, env_idx: int, plan_i: int, texture_idx: int | None
        ) -> None:
            if is_articulation:
        ...
        if self._shared:
            ...
            for target_i, env_idx in enumerate(selected_env_ids):
                _apply(0, env_idx, 0, texture_plan[target_i])
        else:
            for plan_i, env_idx in enumerate(selected_env_ids):
                reuse_i = env_idx if env_idx < num_reuse else 0
                _apply(reuse_i, env_idx, plan_i, texture_plan[plan_i])
    ...
    def _apply_tier_rigid(
        self, segments, env_idx, tier, plan, idx, texture_idx=None
    ) -> None:
        if not segments:
            return
        if tier == 0:  # original
        ...
        if tier == 1 and self._library_textures:  # library
            self._apply_library_props(segments[0].working_inst, plan, idx, texture_idx)
    ...
    def _apply_library_props(self, working_inst, plan, idx, texture_idx=None) -> None:
        tex_idx = texture_idx
        if tex_idx is None:
    ...
    def _apply_tier_articulation(
        self, reuse_i, env_idx, tiers, plan, plan_i, texture_idx=None
    ) -> None:
        ...
        for link_name, segments in link_map.items():
            ...
            if tier == 0:  # original
                ...
                continue
            if tier == 1 and self._library_textures:  # library
                self._apply_library_props(
                    segments[0].working_inst, plan, plan_i, texture_idx
                )
            else:  # solid
    ...
    def _call_legacy(
        ...
        texture_plan = (
            ...
            if self.textures and texture_sampling != "random"
            else [None] * len(selected_env_ids)
        )
