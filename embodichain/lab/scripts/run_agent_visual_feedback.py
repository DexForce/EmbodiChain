...
def test_code(env, code_file_path, check_num=10, kwargs=None):
    ...
    for epid in range(check_num):
        ...
        try:
            ...
            if env.get_wrapper_attr("is_task_success")().item():
                print(f"simulate data episode {epid} success!")
                suc_num += 1
                run_records.append("Success!")
            else:
                print(f"simulate data episode {epid} fail!")
                fail_num += 1
                error_id = 1
    ...
    # ====== Observe at the most frequently occurred error ======
    ...
    if max_error_count > 0:
        ...
        log_dir = kwargs["log_dir"]  # require log_dir
        gen_id = kwargs.get("id", "unknown")  # fallback to a safe string
        episode_id = observe_index
        save_dir = log_dir / "camera_images" / f"{gen_id}_generate_num" / f"episode{episode_id}"
        print(f"Looking for images in: {save_dir}")
        ...
        observation_feedback = env.get_wrapper_attr("validation_agent").validate(step_names, code_content, error_list[observe_index], image_files)
        log_info(f"Observation feedback: {observation_feedback}")
    ...
    return suc_num / check_num, error_list[max_error_index], observation_feedback, max_error_count, run_records
...
def generate_function(
    ...
    if len(error_messages) == 0:
        code_file_path, kwargs, code = env.get_wrapper_attr("generate_code_for_actions")(regenerate=True, log_dir=log_dir, id=id)
    # Generate code based on error status
        ...
        code_file_path, kwargs, code = env.get_wrapper_attr("generate_code_for_actions")(
            ...
            observation_feedbacks=observation_feedbacks, id=id)

    try:
        # Update this section to match the new return values of the run function
        success_rate, error_message, observation_feedback, error_count, run_records = test_code(env, code_file_path, check_num=5, kwargs=kwargs)
        generated_codes.append(code)
        error_messages.append(error_message)
        observation_feedbacks.append(observation_feedback)
        return code, success_rate, error_message, observation_feedback, error_count, run_records
    except KeyboardInterrupt:
        print("Test interrupted by user")
        return code, 0, "Test interrupted by user", None, 10, None
    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error occurred during testing: {e}\n{error_trace}")
        return code, 0, f"Error occurred during testing: {e}", None, 10, None


def main(args, env, gym_config):

    log_info("Start agent data generation with visual feedback.", color="green")
    ...
    log_dir = Path(database_agent_prompt_dir) / args.task_name / "visual_feedback_logs" / timestamp
    ...
    for id in range(generate_num):
        ...
        code, success_rate, error_message, observation_feedback, error_count, run_records = generate_function(
            env, generated_codes, error_messages, observation_feedbacks, log_dir, id=id+1)
