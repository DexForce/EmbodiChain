...
def build_default_registry() -> CapabilityRegistry:
    ...
    for action in (
        ActionCapability("PickUp", frozenset({"object"}), frozenset({"arm"})),
        ActionCapability(
            "MoveHeldObject",
            frozenset({"semantic_goal"}),
            frozenset({"arm"}),
        ),
        ActionCapability(
            "MoveEndEffector",
...
def _expand_coordinated_transport(
    ...
    payloads = goal.get("payloads", [])
    if not isinstance(payloads, Sequence) or isinstance(
        payloads, (str, bytes, bytearray)
    ):
        raise ValueError("coordinated_transport payloads must be a list.")
    if len(payloads) > 4:
        raise ValueError("coordinated_transport supports at most four payloads.")
    normalized_payloads = []
    for index, payload in enumerate(payloads):
        ...
        if uid == object_uid:
            raise ValueError("A coordinated carrier cannot be its own payload.")
        if slot not in {"left", "right", "center", "auto"}:
            raise ValueError(f"Unsupported payload slot {slot!r}.")
        normalized_payloads.append({"object": uid, "slot": slot})
    payload_uids = [item["object"] for item in normalized_payloads]
    if len(payload_uids) != len(set(payload_uids)):
        raise ValueError("coordinated_transport payload objects must be unique.")
