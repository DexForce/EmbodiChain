...
def build_atomic_capability_registry() -> AtomicCapabilityRegistry:
    ...
    unavailable = "The corresponding AtomicAction is not implemented in embodichain.lab.sim.atomic_actions."
    for name, binding, effect in (
        ("Pour", "pour_goal", "preserve_hold"),
        ("PullArticulatedPart", "articulation_goal", "articulation_change"),
        ("PushArticulatedPart", "articulation_goal", "articulation_change"),
        ("TurnKnob", "articulation_goal", "articulation_change"),
    ):
        registry.register(
            AtomicCapability(
                name,
