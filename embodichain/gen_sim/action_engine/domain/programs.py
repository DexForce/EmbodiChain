...
from .motion import MOTION_POLICY_VERSION, validate_motion_policy
...
_BINDING_REQUIREMENTS = {
    ...
    "handover_goal": frozenset({"object"}),
    "handover_staging": frozenset({"object"}),
    "joint_state": frozenset({"source"}),
    "object": frozenset({"object"}),
    "policy_pose": frozenset(),
    "pour_goal": frozenset({"object", "reference_object"}),
    "semantic_goal": frozenset({"semantic_step"}),
    "visual_constraint": frozenset({"camera_uid", "normalized_keypoint"}),
}
...
def _validate_actions(value: Any, edge_context: str) -> list[dict[str, Any]]:
    ...
    for index, raw_action in enumerate(raw_actions):
        ...
        action["motion_policy"] = validate_motion_policy(
            action.get("motion_policy"),
            f"{context}.motion_policy",
        )
        if "seed_node_id" in action:
            _require_nonempty_string(
