...
### Boundary With Embodied Task Program

`embodichain.lab.task_program` remains the public, provider-independent DSL for
authored and configured task programs. It owns Semantic Calls, program
compilation, reusable environment integration, and Gym execution bridges.

GenSim Task Engine and Action Engine serve a different boundary: they generate
task candidates from language or images, select a scene-grounded plan, and
execute a private `SeedGraph` with live replanning and recovery. `TaskDraft`,
`TaskSpec`, and `SeedGraph` are GenSim workflow artifacts; they are not aliases
for `TaskProgramCfg` and are not registered as Task Program language nodes.

The systems share executable behavior only through
`embodichain.lab.sim.atomic_actions`. Neither GenSim engine imports Task Program
language, compiler, runtime, or integration packages. A future conversion must
be an explicit one-way adapter rather than an implicit second public DSL.
