...
class ActionEngineEnv(EmbodiedEnv):
    ...
    def is_object_pressed(
        self,
        uid: str,
        terminal_state: str = "activated",
    ) -> torch.Tensor:
        """Verify a calibrated button state from its live articulation qpos."""
        articulation = self.sim.get_articulation(uid)
        if articulation is None:
            return torch.zeros(int(self.num_envs), dtype=torch.bool, device=self.device)
        settings = self.agent_config.get("articulation_settings", {})
        per_articulation = (
            settings.get(uid, {}) if isinstance(settings, Mapping) else {}
        )
        matches = [
            (name, values)
            for name, values in per_articulation.items()
            if name in articulation.joint_names
