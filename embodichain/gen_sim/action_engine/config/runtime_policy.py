...
RUNTIME_POLICY_SCHEMA: Final = "action_engine_runtime_policy_v2"
...
class RuntimePolicyCfg:
    ...
    grasp: dict[str, Any] = {}
    motion_policies: dict[str, dict[str, Any]] = {}
    ...
    def __post_init__(self) -> None:
        ...
        _require_keys(
            self.execution,
            {"max_transitions", "semantic_step_settle_steps"},
            "execution",
        )
        ...
        _require_keys(self.motion_policies, _MOTION_POLICY_NAMES, "motion_policies")
        if not all(
            isinstance(policy, Mapping) and policy
            for policy in self.motion_policies.values()
        ):
            raise ValueError("Every motion policy must be a non-empty mapping.")
        _require_keys(
    ...
    def as_mapping(self) -> dict[str, Any]:
        ...
        return {
            ...
            "motion_policies": deepcopy(self.motion_policies),
