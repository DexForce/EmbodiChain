...
class SceneExportImporter:
    ...
    def import_scene_and_graph(self) -> tuple[Scene, GeneratedSceneGraph]:
    ...
    def _load_scene_graph(self) -> GeneratedSceneGraph:
    ...
    @staticmethod
    def _scene_graph_from_data(value: object) -> GeneratedSceneGraph:
        """Build a validated ``GeneratedSceneGraph`` from exported graph JSON."""
        if not isinstance(value, dict) or set(value) != {
            ...
            raise ValueError("Generated scene graph must use its versioned schema.")
        if value["schema_version"] != GENERATED_SCENE_GRAPH_SCHEMA:
            raise ValueError("Generated scene graph schema_version is unsupported.")
        if value["artifact_kind"] != "scene_authoring":
            raise ValueError("Generated scene graph must be an authoring artifact.")
        ...
        return GeneratedSceneGraph(nodes=nodes, relations=relations)

    @staticmethod
    def _scene_graph_node_from_data(value: object) -> GeneratedSceneNode:
        if not isinstance(value, dict) or set(value) != {
            "object_id",
            "parent_id",
            "parent_relation",
    ...
    @staticmethod
    def _scene_graph_relation_from_data(value: object) -> GeneratedSceneRelation:
        if not isinstance(value, dict) or set(value) != {
            "source_id",
            "relation",
            "target_id",
        ...
        *,
        ...
        return SceneObject(
            ...
            physics=ObjectPhysics(
                ...
                provenance=self._semantic_text(
                    entry.get("physics_provenance"),
                    field_name=f"{uid}.physics_provenance",
                    default="scene_export/legacy-import",
                ),
            ),
        )
