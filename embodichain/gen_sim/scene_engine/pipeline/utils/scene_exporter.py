...
class SceneExporter:
    ...
    def __init__(
        ...
        *,
        ...
        self.authoring_evidence_path: Path | None = None
    ...
    def export(self) -> Path:
        ...
        authoring_evidence = self._authoring_evidence(
        ...
        scene_config = {
            ...
            "scene_id": self._stable_scene_id(authoring_evidence),
            "background": background_entries,
            "rigid_object": rigid_object_entries,
        }
        ...
        self.authoring_evidence_path = (
            self.export_root / "scene_authoring_evidence.json"
        )
        self.authoring_evidence_path.write_text(
            json.dumps(authoring_evidence, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        log_info(f"Exported scene authoring evidence: {self.authoring_evidence_path}")
    ...
    def _scene_object_config(
        *,
        ...
        return {
            ...
            "physics_provenance": scene_object.physics.provenance,
    ...
    def _authoring_evidence(
        self,
        *,
        exported_entries: dict[str, str],
        z_up_rotation: np.ndarray,
        z_up_pivot_xy: np.ndarray,
    ) -> dict[str, object]:
        """Build deterministic audit evidence without creating runtime identity."""
        nodes_by_id = self.scene_graph.node_by_id()
        objects: list[dict[str, object]] = []
        for scene_object in sorted(self.scene.objects, key=lambda item: item.id):
            node = nodes_by_id[scene_object.id]
            physics = scene_object.physics
            if physics is None:
                raise ValueError(
                    f"Scene object {scene_object.id!r} has no SimReady physics settings."
                )
            exported_object = self._scene_object_y_up_dict(
                scene_object=scene_object,
                z_up_rotation=z_up_rotation,
                z_up_pivot_xy=z_up_pivot_xy,
            )
            relative_glb_path = exported_entries[scene_object.id]
            glb_path = self.export_root / relative_glb_path
            planar_relations = sorted(
                (
                    relation.to_dict()
                    for relation in self.scene_graph.relations
                    if scene_object.id in {relation.source_id, relation.target_id}
                ),
                key=lambda value: (
                    str(value["source_id"]),
                    str(value["relation"]),
            ...
            objects.append(
                {
                    ...
                    "affordance_evidence": {
                        "source": "generated_scene_graph",
                        "orientation_state": node.orientation_state,
                        "table_region": node.table_region,
                        "planar_relations": planar_relations,
                    },
                    "geometry_metadata": {
                        "format": "glb",
                        "frame": "y_up",
                        "asset_path": relative_glb_path,
                        "sha256": self._file_sha256(glb_path),
                        "position": exported_object["pos"],
                        "rotation_xyz_degrees": exported_object["rot"],
                        "scale": exported_object["scale"],
                        "support_surface_z": exported_object["support_surface_z"],
                        "support_contour_xy": exported_object["support_contour_xy"],
                        "support_optimization_rect_xy": exported_object[
                            "support_optimization_rect_xy"
                        ],
                    },
                    "physics_provenance": physics.to_dict(),
                }
            )
        return {
            "schema_version": "scene_authoring_evidence/v1",
            "artifact_kind": "audit_only",
            "objects": objects,
        }

    def _stable_scene_id(self, authoring_evidence: dict[str, object]) -> str:
        """Derive a stable export id from provider-free authoring artifacts."""
        payload = {
            "generated_scene_graph": self.scene_graph.to_dict(),
        ...
        return f"scene-engine-{hashlib.sha256(encoded).hexdigest()[:16]}"

    @staticmethod
    def _file_sha256(path: Path) -> str:
        """Return the content digest for one exported geometry artifact."""
