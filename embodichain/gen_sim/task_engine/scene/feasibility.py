        ...
        *,
        ...
        for step_id, step in steps.items():
            ...
            if task_type == "E8":
                ...
                setting_maps = []
                for uid in raw_uids:
                    entity = objects.get(str(uid), {})
                    attributes = entity.get("attributes", {})
                    joint_settings = (
                        attributes.get("joint_settings", {})
                        if isinstance(attributes, Mapping)
                        else {}
                    )
                    if isinstance(joint_settings, Mapping):
                        setting_maps.extend(
                            list(values)
                            for values in joint_settings.values()
                            if isinstance(values, Sequence)
                            and not isinstance(values, (str, bytes))
                            and values
                        )
                    for evidence in entity.get("affordances", ()):
                        if str(evidence.get("type")) != "turnable":
                            continue
                        parameters = evidence.get("parameters", {})
                        values = (
                            parameters.get("setting_values")
                        ...
                        if (
                            ...
                            setting_maps.append(list(values))
                checks.append(
                    _check(
                        "setting_mapping",
                        step_id,
                        "proven" if len(setting_maps) == 1 else "contradicted",
                        (
                            "One explicit knob setting-to-angle map is available."
                            if len(setting_maps) == 1
                            else "E8 requires exactly one explicit setting_values "
                            "map; ordinal knob settings cannot be inferred from "
                            "joint limits."
                        ),
                        evidence={"setting_map_count": len(setting_maps)},
                    )
                )
