    ...
    *,
    ...
    if all_object_ids:
        ...
        optimized_layout = LayoutManager.optimize_text_layout_slp(
            ...
            xy_sizes={
                object_id: np.asarray(
                    final_items[object_id]["size_xy"], dtype=np.float64
                )
                for object_id in all_object_ids
            },
            initial_centers={
                object_id: initial_centers_all[object_id]
                for object_id in all_object_ids
            },
        ...
        if not anchor_ids:
            anchor_ids = [
                object_id for object_id in all_object_ids if object_id in all_optimized
            ]
