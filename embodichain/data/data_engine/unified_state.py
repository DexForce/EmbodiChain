...
class ActionIndicesGenerator:
    ...
    def get_all_qpos(
        ...
        if handness == ArmEnum.DUAL_ARM.value:
            ...
            return self.get(
                all_names, dof, [handness + qpos_name], [handness + delta_qpos_name]
            )
    ...
    def get_all_delta_qpos(
        ...
        if handness == ArmEnum.DUAL_ARM.value:
            ...
            handness = ControlParts.LEFT_ARM.value
            return self.get(all_names, dof, [handness + delta_qpos_name], [])
        elif handness == ArmEnum.RIGHT_ARM_ONLY.value:
            handness = ControlParts.RIGHT_ARM.value
            return self.get(all_names, dof, [handness + delta_qpos_name], [])
    ...
    def get_all_eef(
        ...
        """Return indices covering end-effector (EEF) related entries.
        ...
        Returns:
            List[int]: Indices corresponding to EEF-related entries.
        """
        global_mapping = self.get_mapping(dof)
        all_names = list(global_mapping.mapping_from_name_to_indices.keys())
        if handness == ArmEnum.DUAL_ARM.value:
            return self.get(
                all_names,
                dof,
                [ControlParts.LEFT_EEF.value, ControlParts.RIGHT_EEF.value],
                [],
            )
        elif handness == ArmEnum.LEFT_ARM_ONLY.value:
