...
#     def _process_and_score(initial_mesh, alignment_type: str):
#         m = initial_mesh.copy()
#         if alignment_type == "pca":
...
#             new_x = closest_axis(x_axis)
#             new_y = closest_axis(y_axis)
#             new_z = closest_axis(z_axis)
#             new_z /= np.linalg.norm(new_z)
#             new_x = new_x - new_z * np.dot(new_x, new_z)
#             new_x /= np.linalg.norm(new_x)
#             new_y = np.cross(new_z, new_x)
#             R_snap = np.column_stack((new_x, new_y, new_z))
#             T_snap = np.eye(4)
#             T_snap[:3, :3] = R_snap
#             m.apply_transform(T_snap)

#         elif alignment_type == "obb":
...
#         for R_flip in generate_discrete_flips():
#             m_candidate = m.copy()

#             T_flip = np.eye(4)
#             T_flip[:3, :3] = R_flip
#             m_candidate.apply_transform(T_flip)

#             z_min = m_candidate.bounds[0][2]
#             m_candidate.apply_translation([0, 0, -z_min])

#             area = compute_support_area(m_candidate)
#             height = m.bounds[1][2] - m.bounds[0][2]
#             score = height - 0.1 * area

#             if score < best_score:
#                 best_score = score
#                 best_mesh = m_candidate

#         return best_mesh, best_score

#     try:
#         mesh_original = mesh.copy()
#         final_mesh_pca, score_pca = _process_and_score(mesh_original, "pca")
#         final_mesh_obb, score_obb = _process_and_score(mesh_original, "obb")
#         if score_pca <= score_obb:
#             print(
#                 f"Selected PCA alignment (Score: {score_pca:.4f} vs OBB: {score_obb:.4f})"
#             )
#             result_mesh = final_mesh_pca
#             STRATEGY = "PCA"
#         else:
#             print(
#                 f"Selected OBB alignment (Score: {score_obb:.4f} vs PCA: {score_pca:.4f})"
#             )
#             result_mesh = final_mesh_obb
#             STRATEGY = "OBB"
...
#         print(f"Alignment failed, fallback. Error: {e}")
...
# ):
#     instruction_text = f"""
# You are a single-purpose multimodal classifier.
# You will be given 6 images of one object rendered from different viewpoints.

# Your task is to identify the ONE image that best shows the object's PRIMARY FUNCTIONAL SURFACE.

# DEFINITION OF PRIMARY FUNCTIONAL SURFACE:
# - The surface that a human would normally face, look at, press, open, aim at, read from, or interact with in standard use.
# - It is the object's main semantic front / operating face, not merely the largest visible face.
# - It is usually the surface that carries the core function of the object.
# - It must be a unique, human-facing, functional surface.
# - Do NOT choose a support/base/bottom surface.
# - Do NOT choose a random side that happens to be large or clear if it is not the functional face.
# - Do NOT choose a view just because it shows more pixels of the object.

# HARD PRIORITY RULE:
# - If the object is a laptop, monitor, smartphone, tablet, camera, TV, oven, speaker, microwave, printer, or similar device, the view showing the FRONT FUNCTIONAL FACE must be preferred.
# - For a laptop specifically: the SCREEN VIEW is the primary surface. The keyboard view is secondary and should NOT be chosen unless the screen is not visible at all and the object is not truly a laptop-like clamshell device.
# - For a monitor / display: choose the view where the screen face is most directly visible.
# - For a smartphone / tablet: choose the view where the screen/front glass is most directly visible.
# - For a camera: choose the lens/front face.
# - For a speaker: choose the grille/front face.
# - For an oven / microwave / cabinet / box with a door: choose the door/front panel face.
# - For a brush / broom / tool with a working head: choose the working head or active end if that is the main interaction face.
# - For objects with a clear labeled front, controls, opening, display, nozzle, spout, or intake/output face, choose that face.

# SELECTION HEURISTICS:
# Prefer the image where the functional surface is:
# 1. most directly facing the camera
# 2. most centered and unobstructed
# 3. most clearly identifiable as the object's real-world front / operating face
# 4. supported by the object's category

# DO NOT BE MISLED BY:
# - top-down views that expose a larger area but are not the real functional front
# - keyboard surfaces on laptops when the screen is available
# - backs, undersides, bases, or support faces
# - symmetric surfaces with no special functional meaning
# - views that show more geometry but less semantic evidence
...
# - If main_surface is not "None", use it as a strong hint for what the functional face is.
# - If orientation_requirement is not "None", use it to infer whether the functional face should be face_up, face_forward, or face_down in normal use.
# - If the hint conflicts with clear visual evidence, trust the images.
# - If the object is category 2, prefer the image that best matches the expected real-world functional orientation, not merely the most visible surface in the render.

# OUTPUT FORMAT:
# Return only one valid JSON object with exactly one field:

# {{
#   "primary_surface_view": string or null
# }}

# RULES:
# - The value must be one of the provided image IDs / view names exactly as given in the input.
# - If the object has no clear primary functional surface, return null.
# - If multiple views show the same surface, choose the one that shows it most directly and most clearly.
# - Do not add any extra text, explanation, markdown, or comments.
# - The JSON must be syntactically valid and parseable.

# IMPORTANT DECISION EXAMPLES:
# - Laptop -> choose the screen view, not the keyboard view.
# - Monitor -> choose the screen/front view.
# - Smartphone -> choose the screen/front-glass view.
# - Camera -> choose the lens/front view.
# - Speaker -> choose the front grille view.
# - Microwave/Oven -> choose the front door/panel view.
# - Brush -> choose the working head / bristle side if that is the functional face.
# - Broom -> choose the sweeping head side if it is the functional face.

# You are selecting the view that best reveals the object's real functional front, not the view that simply shows the object most fully.
# Use the six views together, plus any Additional context text:
...
# CRITICAL TARGET:
# - The correct image is the one where the object's primary surface is DIRECTLY FACING THE VIEWER.
# - In other words, the surface should be as FRONT-FACING / FACE-ON / HEAD-ON as possible.
# - The main surface should look like the object's true front face in the rendered view.
# - Do NOT choose a view where the surface is merely visible from an angle.
# - Do NOT choose a top, side, or bottom view unless that is truly the object's normal front-facing functional face.
# - The goal is to select the image that a human would naturally use when looking straight at the object from its functional front.
...
# - It is usually the surface that carries the core function of the object.
# - It must be a unique, human-facing, functional surface.
# - It must be the surface that is most naturally "facing us" in normal use.
# - Do NOT choose a support/base/bottom surface.
# - Do NOT choose a random side that happens to be large or clear if it is not the functional face.
# - Do NOT choose a view just because it shows more pixels of the object.
...
# - If the object is a laptop, monitor, smartphone, tablet, camera, TV, oven, speaker, microwave, printer, or similar device, the view showing the FRONT FUNCTIONAL FACE must be preferred.
# - For a laptop specifically: the SCREEN VIEW is the primary surface. The keyboard view is secondary and should NOT be chosen unless the screen is not visible at all and the object is not truly a laptop-like clamshell device.
# - For a monitor / display: choose the view where the screen face is most directly visible and most face-on.
# - For a smartphone / tablet: choose the view where the screen/front glass is most directly visible and most face-on.
# - For a camera: choose the lens/front face that is most directly facing the viewer.
# - For a speaker: choose the grille/front face that is most directly facing the viewer.
# - For an oven / microwave / cabinet / box with a door: choose the door/front panel face that is most directly facing the viewer.
# - For a brush / broom / tool with a working head: choose the working head or active end if that is the main interaction face, but only if it is the face that would be naturally presented toward the user.
# - For objects with a clear labeled front, controls, opening, display, nozzle, spout, or intake/output face, choose the image where that face is most directly facing the viewer.

# SELECTION HEURISTICS:
# Prefer the image where the functional surface is:
# 1. most directly facing the camera / viewer
# 2. least foreshortened
# 3. most centered and unobstructed
# 4. most clearly identifiable as the object's real-world front / operating face
# 5. most consistent with the object's category

# DO NOT BE MISLED BY:
# - top-down views that expose a larger area but are not the real front face
# - keyboard surfaces on laptops when the screen is available
# - backs, undersides, bases, or support faces
# - symmetric surfaces with no special functional meaning
# - views that show more geometry but less semantic evidence
# - views where the right surface exists but is shown at an angle instead of facing us directly
...
# - The value must be one of the provided image IDs / view names exactly as given in the input.
# - If the object has no clear primary functional surface, return null.
# - If multiple views show the same surface, choose the one where that surface is most directly facing the viewer and most face-on.
# - Do not add any extra text, explanation, markdown, or comments.
# - The JSON must be syntactically valid and parseable.
...
# - Speaker -> choose the front grille view.
# - Microwave/Oven -> choose the front door/panel view.
# - Brush -> choose the working head / bristle side if that is the functional face, but only when it is the face most directly facing the viewer.
# - Broom -> choose the sweeping head side if it is the functional face, but only when it is the face most directly facing the viewer.

# You are selecting the view that best reveals the object's real functional front as a face-on view, not the view that simply shows the object most fully.
# Use the six views together, plus any Additional context text:
# {extra_text if extra_text else "None"}
# """
