...
def build_gym_policy_runtime(
    ...
    *,
    ...
    try:
        ...
        actor_obs_groups, critic_obs_groups = resolve_policy_obs_groups(
            config["policy"]
        )
        actor_observation_dim = int(
            flatten_observation_groups(
                sample_observation_td,
                actor_obs_groups,
            ).shape[-1]
        )
        ...
        policy = _build_gym_policy(
            ...
            uses_separate_critic_obs=uses_separate_critic_obs,
    ...
    *,
    ...
    if policy_name == "actor_critic":
        ...
        return build_policy(
            ...
            env.action_space,
            device,
            actor=build_mlp_from_cfg(actor_cfg, actor_observation_dim, action_dim),
            critic=build_mlp_from_cfg(critic_cfg, critic_observation_dim, 1),
            critic_obs_space=(
                critic_observation_dim if uses_separate_critic_obs else None
            ),
        )
...
def _build_learning_policy(
    ...
    actor_observation_dim = _observation_space_dim(
    ...
    policy = build_policy(
        ...
        (
            actor_observation_dim
            if policy_block["name"].lower() in {"actor_critic", "actor_only"}
            else env.single_observation_space
        ),
        env.single_action_space,
        device,
        actor=(
            build_mlp_from_cfg(actor_cfg, actor_observation_dim, action_dim)
            if actor_cfg is not None
            else None
        ),
        critic=(
            build_mlp_from_cfg(critic_cfg, critic_observation_dim, 1)
            if critic_cfg is not None
            else None
        ),
        critic_obs_space=(critic_observation_dim if uses_separate_critic_obs else None),
    )
