...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--keymap", choices=("wasd", "arrows"))
...
def _validate_native_options(args: argparse.Namespace) -> None:
    ...
    if not args.viewer and (
        ...
        or args.keymap is not None
        or args.termination_behavior is not None
    ):
        raise ValueError(
            "--control-steps, --duration, --command, --keymap, and "
            "--termination-behavior require --viewer"
        )
