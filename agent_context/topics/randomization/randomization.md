...
Basic uniform randomizers use `embodichain.utils.math.sample_uniform(lower, upper, size)` for uniform sampling.
## Common Failure Modes

| Symptom | Likely cause |
|---|---|
| Randomizer silently does nothing | Check UID resolution and the selected randomizer: unsupported target types or no-op parameters can skip work |
