...
registered roots or links through public pose reads on the simulation thread. Native rendering uses DexSim's `Arena.create_debug_mesh` support;
browser rendering carries meshes in `SceneOverlays`. `draw_marker()` remains
...
| Native marker creation reports missing debug-mesh support | Use a DexSim build whose arena implements `create_debug_mesh`, or run with `--viser`. Hybrid, FastRT, and OfflineRT native engines use that API. |
