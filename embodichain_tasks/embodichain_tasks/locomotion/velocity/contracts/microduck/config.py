...
__all__ = ["MicroDuckVelocityConfig", "load_config"]
...
def load_config() -> MicroDuckVelocityConfig:
    ...
    """
    data = load_task_data("microduck_flat")
    robot = data["robot"]
    ...
    if not len(joint_names) == len(default_position) == len(action_scale):
        raise ValueError("MicroDuck joint, default pose, and action dimensions differ")
    return MicroDuckVelocityConfig(
        data=data,
