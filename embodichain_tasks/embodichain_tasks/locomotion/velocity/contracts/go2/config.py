...
__all__ = ["Go2VelocityConfig", "load_config"]


@configclass
class Go2VelocityConfig:
...
def load_config() -> Go2VelocityConfig:
    ...
    if not len(joint_names) == len(default_position) == len(action_scale):
        raise ValueError("Go2 joint, default pose, and action dimensions differ")
    return Go2VelocityConfig(
        data=data,
