...
__all__ = ["H12VelocityConfig", "load_config"]


@configclass
class H12VelocityConfig:
...
def load_config() -> H12VelocityConfig:
    ...
    if not len(joint_names) == len(default_position) == len(action_scale):
        raise ValueError("H1_2 joint, default pose, and action dimensions differ")
    return H12VelocityConfig(
        data=data,
