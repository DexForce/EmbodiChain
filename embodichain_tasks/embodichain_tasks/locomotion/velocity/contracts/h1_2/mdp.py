...
from .config import H12VelocityConfig
...
def action_target(config: H12VelocityConfig, action: torch.Tensor) -> torch.Tensor:
    ...
    if action.shape[-1] != config.action_dim:
        raise ValueError(
            f"H1_2 expected {config.action_dim} actions, got {action.shape[-1]}"
        )
...
def build_observations(
    config: H12VelocityConfig, state: H12State
    ...
    if actor.shape[-1] != config.actor_observation_dim:
        raise RuntimeError(
            f"H1_2 actor observation has {actor.shape[-1]} values, "
            f"expected {config.actor_observation_dim}"
        )
    if critic.shape[-1] != config.critic_observation_dim:
        raise RuntimeError(
            f"H1_2 critic observation has {critic.shape[-1]} values, "
...
def compute_rewards(
    config: H12VelocityConfig, state: H12State, terminated: torch.Tensor
) -> H12Reward:
    ...
    return H12Reward(raw_terms=raw, weighted_terms=weighted, total=total)
