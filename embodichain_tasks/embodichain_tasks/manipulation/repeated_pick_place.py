...
__all__ = ["RepeatedPickPlaceRlinfEnv"]
...
class RepeatedPickPlaceRlinfEnv(EmbodiedEnv):
    ...
    def reset(self, *args: Any, **kwargs: Any) -> tuple[EnvObs, dict[str, Any]]:
        ...
        reset_ids = None if options is None else options.get("reset_ids")
        if reset_ids is None:
            self._success_hold_steps.zero_()
        else:
            reset_ids = torch.as_tensor(reset_ids, dtype=torch.long, device=self.device)
