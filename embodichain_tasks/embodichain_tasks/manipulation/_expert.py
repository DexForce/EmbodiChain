...
"""Shared Expert Program helpers for manipulation tasks."""
...
def _task_config_path(
    ...
    *relative_parts: str,
    ...
    """Resolve task package data in an installed wheel or source checkout."""
    segments = (domain, task_name, *relative_parts)
    if not all(
        type(segment) is str
        and segment
        ...
        raise ValueError("task config path segments must be non-empty strings.")
    relative_path = Path("configs", "tasks", *segments)
    for parent in Path(__file__).resolve().parents:
        candidate = parent / relative_path
        if candidate.is_file():
            return candidate
    raise FileNotFoundError(f"Packaged task config is missing: {relative_path}")


def load_bundled_expert_program(domain: str, task_name: str) -> ExpertProgramCfg:
    """Load one task-local Expert Program from package data.

    Args:
        domain: Task domain below ``configs/tasks``.
        task_name: Task directory within the domain.

    Returns:
        Decoded Expert Program configuration.
    """
    return load_expert_program(
        _task_config_path(domain, task_name, "expert", "program.yaml")
    )
