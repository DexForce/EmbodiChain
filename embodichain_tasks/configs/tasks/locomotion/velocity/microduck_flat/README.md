...
Track planar velocity and yaw-rate commands with the MicroDuck biped on flat terrain.
Gym ID: `MicroDuckFlatRL-v1`.
...
```bash
embodichain show-task embodichain_tasks:microduck_flat
embodichain run-env --gym_config embodichain_tasks/configs/tasks/locomotion/velocity/microduck_flat/env.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/microduck_flat/agents/ppo.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/microduck_flat/agents/ppo.newton.yaml
```
...
Check command tracking, tilt, trunk clearance, body contacts and episode survival.
