# Unitree Go1 Flat Velocity

Track planar velocity and yaw-rate commands with the Unitree Go1 quadruped on flat terrain.
Gym ID: `UnitreeGo1FlatRL-v1`.
...
```bash
embodichain show-task embodichain_tasks:go1_flat
embodichain run-env --gym_config embodichain_tasks/configs/tasks/locomotion/velocity/go1_flat/env.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/go1_flat/agents/ppo.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/go1_flat/agents/ppo.newton.yaml
```
...
Check command tracking, tilt failures, foot clearance and episode survival.
