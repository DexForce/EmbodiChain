# Unitree Go2 Flat Velocity

Track planar velocity and yaw-rate commands with the Unitree Go2 quadruped on flat terrain.
Gym ID: `UnitreeGo2FlatRL-v1`.
...
```bash
embodichain show-task embodichain_tasks:go2_flat
embodichain run-env --gym_config embodichain_tasks/configs/tasks/locomotion/velocity/go2_flat/env.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/go2_flat/agents/ppo.yaml
embodichain train-rl --config embodichain_tasks/configs/tasks/locomotion/velocity/go2_flat/agents/ppo.newton.yaml
```
