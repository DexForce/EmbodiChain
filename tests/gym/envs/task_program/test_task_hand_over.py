...
from embodichain.lab.task_program import ObjectNearTargetValidatorCfg, SegmentCfg
...
def _configured_env_cfg():
    """Parse the production config and ensure its integration ID is registered."""
...
def test_hand_over_profile_binds_unified_left_to_right_transfer() -> None:
    ...
    binding = _integration().registration.robot_profile_binding
...
def test_hand_over_program_preflights_against_configured_registration() -> None:
    """The packaged semantic program statically links through its integration."""
    cfg = _configured_env_cfg()
    assert cfg.task_program is not None
    registration = _integration().registration

    compiled = registration.catalog.preflight(cfg.task_program)
...
def _run_real_sim_expert_episode() -> dict[str, object]:
    ...
    assert cfg.task_program is not None
    program = cfg.task_program.program
...
@pytest.mark.gpu
@pytest.mark.slow
def test_real_sim_expert_episode_reports_runtime_and_validation(
    tmp_path: Path,
) -> None:
