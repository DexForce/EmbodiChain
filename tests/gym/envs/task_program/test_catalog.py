...
"""Tests for task-registration-owned Task Program integration catalogs."""
...
from embodichain.lab.task_program.semantics.effects import EffectMonitorRef
...
def create_cube_scene_binding() -> SimulationSceneBinding:
    ...
    return _cube_integration().registration.scene_binding
...
_CUBE_REFERENCE_INTEGRATION = _cube_integration()
CUBE_ROBOT_PROFILE_ID = (
    _CUBE_REFERENCE_INTEGRATION.registration.robot_profile_binding.profile_id
)
CUBE_SCENE_REGISTRY_ID = (
    _CUBE_REFERENCE_INTEGRATION.registration.scene_binding.registry_id
)
...
class _CatalogRegisteredLowererFactory:
    """Frozen declaration creating a fresh lowerer per integration assembly."""
...
class _OpaqueHandOverPoseProvider(HandOverPoseProvider):
    ...
    def resolve(
        ...
        *,
        ...
        del call, context, bound
        raise AssertionError("Opaque providers must never reach integration.")
...
class _CatalogControlPartEvidenceFactory:
    ...
    def create(
        ...
        *,
        ...
        """Return one fresh provider for every integration assembly."""
...
class _CatalogSceneProvider:
    """Structurally satisfy the scene-provider integration boundary."""
...
def _registration() -> SimulationTaskProgramRegistration:
    """Build one isolated provider-free task registration."""
    return SimulationTaskProgramRegistration(
...
def _place_relation_catalog(
    *,
    ...
    return TaskProgramIntegrationCatalog(
...
def test_parallel_safety_factory_must_return_a_validator() -> None:
    """A malformed registration-owned factory fails before integration dispatch."""
...
def test_catalog_rejects_linked_place_relation_without_exact_grounder() -> None:
    """A linked affordance cannot defer a missing typed grounder to integration."""
...
def test_fingerprint_covers_workflow_recovery_policy() -> None:
    """A recovery budget is immutable registration-owned integration behavior."""
    base = create_cube_robot_profile_binding().presets[0]
...
def test_fingerprint_is_independent_of_catalog_and_provider_insertion_order() -> None:
    ...
    forward = SimulationTaskProgramRegistration(
        **common,
    ...
    reversed_registration = SimulationTaskProgramRegistration(
        **common,
...
def test_fingerprint_owns_provider_ids_and_declarative_fields() -> None:
    ...
    changed_value = SimulationTaskProgramRegistration(
        scene_binding=create_cube_scene_binding(),
        robot_profile_binding=create_cube_robot_profile_binding(),
        relation_grounders=(_CatalogRelationGrounder(),),
...
def test_env_spec_keeps_typed_registration_out_of_gym_kwargs() -> None:
    ...
    spec = EnvSpec(
        ...
        task_program_registration=registration,
    )

    assert spec.task_program_registration is registration
    assert spec.gym_spec.kwargs == {"physical_option": 3}


def test_env_spec_derives_registration_and_injects_runtime_factory() -> None:
    """One integration declaration owns static preflight and live environment binding."""
    ...
    registration = _registration()
    factory = SimulationTaskProgramAdapterFactory(registration)
    spec = EnvSpec(
        "RuntimeFactoryTest-v1",
        _Environment,
        default_kwargs={"physical_option": 3},
        task_program_adapter_factory=factory,
    )

    environment = spec.make(cfg=object())

    assert spec.task_program_registration is registration
    assert spec.task_program_adapter_factory is factory
    assert environment.kwargs["task_program_adapter_factory"] is factory
    assert spec.gym_spec.kwargs == {"physical_option": 3}

    with pytest.raises(ValueError, match="cannot be overridden"):
        spec.make(
            cfg=object(),
            task_program_adapter_factory=SimulationTaskProgramAdapterFactory(
                registration
            ),
        )
