...
"""Tests for reusable environment-backed Task Program assembly."""
...
from embodichain.lab.task_program.compiler.lowering import SemanticCallCompiler
from embodichain.lab.task_program.semantics.evidence import EffectEvidenceProvider
...
class _FakeDeclarativeEnvironment(EmbodiedEnv):
    ...
    def __init__(self, adapter: TaskProgramEnvironmentAdapter) -> None:
...
def test_embodied_env_delegates_compilation_and_bridge_assembly() -> None:
    ...
    compiled = env.compile_task_program(_program())

    assert factory.calls == Counter(scene=1)
    bridge = env.create_task_program_bridge(compiled)
    assert isinstance(bridge, TaskProgramDemoBridge)
...
def test_later_post_policy_requires_port_before_runtime_assembly() -> None:
    ...
    with pytest.raises(TaskProgramBridgeError, match="SegmentPostPolicyPort"):
        adapter.create_bridge(compiled)
...
def test_later_validator_requires_port_before_runtime_assembly() -> None:
    ...
    with pytest.raises(TaskProgramBridgeError, match="SegmentValidatorPort"):
        adapter.create_bridge(compiled)
...
def test_factory_protocol_is_required() -> None:
    ...
    with pytest.raises(TypeError, match="TaskProgramEnvironmentFactory"):
        TaskProgramEnvironmentAdapter(object(), step_dt=_STEP_DT)
