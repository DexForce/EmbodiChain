...
def test_dual_franka_profile_defines_robot_runtime_and_observation_contracts() -> None:
    ...
    assert robot["solver_cfg"]["left_arm"]["class_type"] == "PinocchioSolver"
    assert robot["solver_cfg"]["right_arm"]["class_type"] == "PinocchioSolver"
    assert len(robot["control_parts"]["left_arm"]) == 7
    assert len(robot["control_parts"]["right_arm"]) == 7
    assert robot["control_parts"]["left_eef"] == ["left_finger_joint[1-2]"]
    assert robot["control_parts"]["right_eef"] == ["right_finger_joint[1-2]"]
    assert len(robot["init_qpos"]) == 18
