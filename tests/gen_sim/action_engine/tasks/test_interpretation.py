...
def test_missing_e3_target_repair_preserves_pour_semantics() -> None:
    invalid = {
        "steps": [
            _step(
                "pour",
                "E3",
                _selector("scene_ref", reference="the source cup"),
                relation="above",
            )
        ]
    }
    ...
    with pytest.raises(ValueError, match="after one repair"):
        task_interpretation_module.interpret_instruction_draft(
            "Grab the cup and pour its contents into the bin.",
            model="test-model",
            caller=caller,
        )
...
def test_missing_e6_object_reaches_targeted_repair() -> None:
    ...
    repaired["steps"][0]["object"] = _selector("scene_ref", reference="the drawer")
    prompts: list[str] = []

    def caller(**kwargs):
        prompts.append(kwargs["prompt"])
        return deepcopy(invalid if len(prompts) == 1 else repaired)

    result = task_interpretation_module.interpret_instruction_draft(
        "Open the drawer with the right arm.",
        model="test-model",
        caller=caller,
    )
