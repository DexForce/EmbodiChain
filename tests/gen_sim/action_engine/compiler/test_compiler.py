...
def test_place_relative_carries_payloads_through_single_arm_action_bindings() -> None:
    execution = compile_task_agent(
        _program(
            {
                "id": "s01_place_carrier",
                "operator": "place_relative",
                "object": "paper_cup",
                "goal": {
                    "reference_object": "popcorn_bucket",
                    "relation": "on",
                    "payloads": [{"object": "glue_stick", "slot": "center"}],
                },
            }
        )
    )
    ...
    assert step["goal"]["payloads"] == [{"object": "glue_stick", "slot": "center"}]
    carrying_actions = [
        action
        for edge in execution["edges"]
        for action in edge["actions"]
        if action["atomic_action_class"] in {"PickUp", "MoveHeldObject", "Place"}
    ]
    assert carrying_actions
    assert all(
        action["target_binding"]["payloads"] == step["goal"]["payloads"]
        for action in carrying_actions
    )
...
def test_orient_object_composes_upright_motion_modifier() -> None:
    execution = compile_task_agent(
        {
            "schema_version": TASK_AGENT_SCHEMA,
            "task": "upright",
