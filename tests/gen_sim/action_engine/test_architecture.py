...
_TASK_ENGINE_ROOT = _PACKAGE_ROOT.parent / "task_engine"
...
def test_gen_sim_engines_do_not_import_task_program_runtime() -> None:
    offenders: list[str] = []
    for package_root in (_PACKAGE_ROOT, _TASK_ENGINE_ROOT):
        for path in sorted(package_root.rglob("*.py")):
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    ...
                    continue
                if any(
                    name.startswith("embodichain.lab.task_program") for name in names
                ):
                    offenders.append(path.relative_to(_PACKAGE_ROOT.parent).as_posix())
                    break
    assert offenders == []
