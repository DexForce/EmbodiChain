...
class _BranchRuntime:
    ...
    def step(self) -> SkillResult:
        ...
        if scripted.hold_targets:
            ...
            scripted.status is not SkillStatus.RUNNING and self._emit_terminal_hold
        ):
...
def test_parallel_runtime_merges_one_frame_and_hold_pads_short_lane() -> None:
    ...
    assert isinstance(left.runtime, _BranchRuntime)
    assert isinstance(right.runtime, _BranchRuntime)
    first_lane_steps = (left.runtime.step_count, right.runtime.step_count)
    same_tick = runtime.step()
    assert same_tick.wait_duration == pytest.approx(0.1)
    assert outbound.operations == ["send"]
    assert (left.runtime.step_count, right.runtime.step_count) == first_lane_steps
    ...
    assert len(outbound.frames) == 1
    branch_steps = (left.runtime.step_count, right.runtime.step_count)
    same_tick = runtime.step()
    assert same_tick.wait_duration == pytest.approx(0.1)
    assert outbound.operations == ["send", "hold"]
    assert (left.runtime.step_count, right.runtime.step_count) == branch_steps
...
def test_deferred_command_waits_for_clock_after_padding_hold() -> None:
    ...
    runtime.start()
    padded = runtime.step()
    lane_steps = (left.runtime.step_count, right.runtime.step_count)
    same_tick = runtime.step()
    ...
    assert (left.runtime.step_count, right.runtime.step_count) == lane_steps
...
def test_completion_hold_waits_for_clock_after_accepted_command() -> None:
    ...
    assert same_tick.status is SkillStatus.RUNNING
    assert same_tick.wait_duration == pytest.approx(0.1)
    assert outbound.operations == ["send"]
    assert (left.runtime.step_count, right.runtime.step_count) == lane_steps
...
def test_parallel_timeout_counts_completed_environment_steps() -> None:
    ...
    assert before_step.status is SkillStatus.RUNNING
    assert before_step.elapsed_steps == 0

    clock.time = 0.1
    timed_out = runtime.step()
    assert timed_out.status is SkillStatus.FAILED
...
def test_parallel_cancel_discards_deferred_frame_and_covers_started_rows() -> None:
    ...
    assert cancelled.status is SkillStatus.CANCELLED
...
def test_deferred_frame_validation_failure_does_not_advance_lanes() -> None:
    ...
    assert isinstance(left.runtime, _BranchRuntime)
    assert isinstance(right.runtime, _BranchRuntime)
    steps_before_dispatch = (left.runtime.step_count, right.runtime.step_count)

    clock.time = 0.1
    failed = runtime.step()

    assert failed.status is SkillStatus.FAILED
    assert (left.runtime.step_count, right.runtime.step_count) == steps_before_dispatch
    assert not outbound.frames
    assert outbound.operations == ["hold", "cancel", "hold"]
...
@pytest.mark.parametrize(
    ...
    [
        ({}, SkillStatus.CANCELLED),
        ({"reject_cancel": True}, SkillStatus.FAILED),
        ({"reject_hold": True}, SkillStatus.FAILED),
    ],
)
def test_parallel_caller_cancel_checks_cancel_and_hold_acknowledgements(
    ...
    assert outbound.cancels == 1
    assert outbound.holds >= 1
    if expected_status is SkillStatus.CANCELLED:
