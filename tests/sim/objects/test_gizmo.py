...
class _FakeRobot:
    """Small Robot-compatible state holder for adapter tests."""
...
def test_robot_adapter_reads_root_and_link_pose_through_robot() -> None:
    adapter = _RobotGizmoAdapter(_FakeRobot(), "arm")
...
def test_robot_ik_chain_can_be_configured_without_embodichain_solver() -> None:
    ...
    root_link, end_link, tcp_pose = gizmo._resolve_robot_ik_chain(robot)
...
def test_destroy_removes_gizmo_from_dexsim_environment() -> None:
    ...
    assert environment.removed is dexsim_gizmo
    assert dexsim_gizmo.detached is True
