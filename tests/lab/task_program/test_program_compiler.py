...
"""Tests for provider-free Task Program compilation and lazy expansion."""
...
class _NeverObserveProvider:
    ...
    def observe(
        ...
        *,
        ...
        self.calls += 1
        raise AssertionError("Task Program compilation must not observe state.")
...
def _program(
    ...
    *,
    ...
    """Build one valid Task Program around a supplied node."""
...
def test_compiled_program_builds_cross_segment_analysis_windows_provider_free() -> None:
    ...
    assert type(compiled) is CompiledTaskProgram
...
def test_wait_stable_accepts_every_canonical_scene_entity_subtype() -> None:
    ...
    segment = next(
        iter(TaskProgramCompiler.from_scene_registry(registry).compile(config))
    )
...
def test_compiler_rechecks_mutated_repeat_and_target_bounds() -> None:
    ...
    with pytest.raises(TaskProgramCompileError) as repeat_error:
        TaskProgramCompiler.from_scene_registry(registry).compile(config)
    ...
    with pytest.raises(TaskProgramCompileError) as target_error:
        TaskProgramCompiler.from_scene_registry(registry).compile(config)
