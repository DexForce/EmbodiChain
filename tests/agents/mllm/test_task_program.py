...
"""Tests for the strict MLLM Task Program frontend."""
...
def _integration() -> TaskProgramIntegrationCfg:
...
def _invoke(call: dict[str, object]) -> dict[str, object]:
    """Wrap one semantic call in an Task Program invoke node."""
...
def _adapter(factory: _CompileOnlyFactory) -> TaskProgramEnvironmentAdapter:
    """Create the existing production adapter around the compile-only factory."""
    return TaskProgramEnvironmentAdapter(factory, step_dt=0.02)
...
def test_decoder_propagates_strict_json_failures(response: str, code: str) -> None:
    with pytest.raises(TaskProgramDecodeError) as error:
        decode_mllm_task_program(response, integration=_integration())
...
def test_compile_frontend_reuses_existing_adapter_and_compiler() -> None:
    ...
    model_compiled = compile_mllm_task_program(
        response,
        adapter=adapter,
        integration=_integration(),
    )
    ...
    direct_compiled = adapter.compile(decode_task_program(direct_data))

    model_call = list(model_compiled)[0].calls[0].call
    direct_call = list(direct_compiled)[0].calls[0].call
    assert type(model_compiled) is CompiledTaskProgram
