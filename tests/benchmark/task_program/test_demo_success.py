...
from scripts.benchmark.task_program import demo_success as demo_success_module
...
def test_zero_case_and_zero_trial_benchmarks_are_rejected(tmp_path: Path) -> None:
    ...
    empty_raw.write_text(
        json.dumps(
            {
                "schema_version": 1,
                "benchmark": "task_program_demo_success",
                "trials": [],
            }
        ),
        encoding="utf-8",
...
def test_gym_runner_reuses_one_environment_and_shared_no_retry_harness(
    ...
    artifacts = run_gym_demo_success_benchmark(
        ...
        task_program_path=tmp_path / "program.yaml",
...
def test_gym_environment_builder_uses_standard_public_config_pipeline(
    ...
    assert env_cfg.task_program is program
    assert calls == [
        "discover",
        "hooks",
        ("build", launcher_args),
...
def test_cli_live_mode_dispatches_fixed_seed_case(
    ...
    assert captured["task_program_path"] == Path("program.yaml")
