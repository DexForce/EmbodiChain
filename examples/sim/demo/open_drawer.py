...
def create_robot(sim: SimulationManager) -> Robot:
    ...
    gripper_attach_xpos[:3, :3] = R.from_rotvec([0, 0, 0], degrees=True).as_matrix()
    ...
    return sim.add_robot(
        cfg=RobotCfg(
            ...
            drive_pros=JointDrivePropertiesCfg(
                ...
                max_velocity={"JOINT[0-9]": 1.0, "FINGER[1-2]": 0.5},  # 设置最大速度限制（rad/s）
                drive_type="force",
            ),
            solver_cfg={
...
def create_cabinet(sim: SimulationManager, cabinet_path: str = None) -> Articulation:
    ...
    """
    cabinet_urdf_path = "/home/dex/桌面/forvisualization_examples_PM_datasets/StorageFurniture@46549@18/object_fromJson2urdf_decomposed.urdf"
    ...
    return sim.add_articulation(
        cfg=ArticulationCfg(
            ...
            drive_pros=JointDrivePropertiesCfg(
                stiffness=2.0, damping=1.0, max_effort=100.0, drive_type="force"
            ),
            fix_base=True,
        )
    )
...
def open_cabinet_drawer(sim: SimulationManager, robot: Robot, cabinet: Articulation):
    """
    ...
    grasp_pos = cabinet_position + torch.tensor([-0.52, 0.02, 0.33], device=sim.device)
...
def main():
    ...
    Initializes the simulation, creates the robot and cabinet, and performs the open drawer task.
    """
    ...
    sim.update(step=100)
    open_cabinet_drawer(sim, robot, cabinet)
